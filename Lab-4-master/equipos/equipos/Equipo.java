...
public class Equipo{
    ...
    public int valorHoraEstimado() throws EquipoExcepcion{
        ...
        for (int j=0; j<equipoHoras.size(); j++){
            if (equipoHoras.get(j)==-1){
                equipoHoras.set(j,promedio);
            }
        }
        if (mitadEquipo>=lenghtList/2){
            int suma=0;
            for (int i=0; i<personas.size(); i++){
                String a = (personas.get(i).getNombre());
    ...
    public int valorHoraAsumido() throws EquipoExcepcion {
        ...
        int primerValor=equipoHoras.get(0);
        int ultimoValor=equipoHoras.get(equipoHoras.size()-1);
        for (int i=0; i<personas.size(); i++){
            try {
            String a =(personas.get(i).getNombre());
            personas.get(i).valorHora();
            } catch (EquipoExcepcion e){ 
                String a =e.VALOR_DESCONOCIDO;
                if (a =="El valor no es conocido"){
                   equipoHoras.add(ultimoValor);
                }
                else if( a =="La persona no es conocida"){
